...
class DecisionController {private final DecisionService service;private final RealityLogRepository logs;
 @PostMapping ResponseEntity<?> create(@Valid @RequestBody DecisionRequest r,@RequestHeader(value="X-Bandersnatch-Simulate",required=false)String simulate,Authentication a){return ResponseEntity.accepted().body(Views.decision(service.create(r,a,simulate)));}
 ...
 @GetMapping Object all(@RequestParam(required=false)String branchType,@RequestParam(required=false)String impactLevel,@RequestParam(required=false)String status,@RequestParam(required=false)Long playthroughId,@RequestParam(defaultValue="1")int page,@RequestParam(defaultValue="10")int size,Authentication a){Page<Decision>x=service.list(branchType,impactLevel,status,playthroughId,page,size,a);return Map.of("content",x.getContent().stream().map(Views::decision).toList(),"totalElements",x.getTotalElements(),"totalPages",x.getTotalPages(),"currentPage",Math.max(1,page),"size",x.getSize());}
}
